...
APP_NAME=ads-dash
VERSION=1.0
#DOCKER_REPO=asia.gcr.io/pelagic-pager-288405
...
run: ## Run container
	docker run -i -t --rm --env-file .env -p $(PORT):$(DIST_PORT) --name="$(APP_NAME)" $(APP_NAME)

run-dev: ## Run container dev volume
	docker run -i -t --rm --env-file .env -v $(PWD)/:/var/www/app -p $(PORT):$(DIST_PORT) --name="$(APP_NAME)" $(APP_NAME)
